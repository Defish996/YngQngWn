...
namespace ns_control
{
    ...
    public:
        ...
        {
            // 三个参数分别是 题号(要找哪一道题) 客户上传上来的想验证这道题的代码 想返回的结果
            // LOG(DEBUG) << in_json << " \nnumber:" << number << "\n";
            ...
            Json::Value compile_value;// 将这个序列化之后发送到远端
            compile_value["input"] = in_value["input"].asString();// 该行代码将客户传入的input值从in_value字典中取出，并将其转换为字符串形式，然后赋值给compile_value字典中的"input"键。主要功能是传递输入数据给编译服务器。将客户传进来的input直接传给编译服务器
